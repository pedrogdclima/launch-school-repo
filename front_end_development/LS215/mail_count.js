...
function findRange(emailData) {
  let datesArray = emailData.map(email => {
    return Date.parse(email.split('#/#')[2].slice(7))
  });
  return [new Date(Math.max(...datesArray)), new Date(Math.min(...datesArray))];
}

function mailCount(emailData) {
  let emailArray = emailData.split('##||##');
  let emailCount = emailArray.length;
  let dateRange = findRange(emailArray).map(date => date.toDateString());

  console.log(
    `Count of Email: ${emailCount}`,
    '\n',
